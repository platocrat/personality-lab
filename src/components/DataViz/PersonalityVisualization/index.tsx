...
const PersonalityVisualization = ({ data, averages }) => {
  const [activeFacet, setActiveFacet] = useState('Self-Management Skills')
  const svgRef = useRef<any>(null)

  // Calculating averages is not depicted in this snippet
  const averageScore = averages[activeFacet]
  ...
  useEffect(() => {
    ...
    const facetNames = Object.keys(data.facetScores)
    const domainNames = facetToDomainMapping[activeFacet]
    ...
    svg.selectAll('.outer-path')
      ...
      .attr('d', outerCircle)
      .attr('fill', (d) => d === activeFacet ? '#ccc' : '#ccc')
      .attr('stroke', 'white')
      // Make the active facet slightly opaque
      .attr('opacity', (d) => d === activeFacet ? 0.5 : 1)
      .style('cursor', 'pointer') // Change cursor to pointer on hover
      .on('click', (e, d) => setActiveFacet(d)) // corrected line
      .on('mouseover', function (event, d) {
        // Increase opacity on hover, but keep active facet distinct if it is 
        // the one being hovered
        d3.select(this).attr('opacity', d === activeFacet ? 0.4 : 0.7)
      })
      .on('mouseout', function (event, d) {
        // Reset opacity, maintaining active facet distinction
        d3.select(this).attr('opacity', d === activeFacet ? 0.5 : 1)
      })
    ...
    svg.selectAll('.inner-text')
      ...
      .attr('text-anchor', 'middle')
      .text((d: any) => data.domainScores[d])
    ...
    svg.append('text')
      ...
      .attr('text-anchor', 'middle')
      .attr('font-size', '48px')
      .text(data.facetScores[activeFacet])
